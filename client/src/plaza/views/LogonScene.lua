...
function LogonScene.onceExcute()
    ...
    --IP
    appdf.onHttpJsionTable("http://2018.ip138.com/ic.asp","GET",nil,function(jstable,jsdata)
    	dump(jsdata)
    	print(type(jsdata))
...
function LogonScene:onCreate()
     ...
     if device.platform == "android" then
        local function onrelease(code, event)
            ...
            if code == cc.KeyCode.KEY_BACK then
                ...
                self._dialog1 = QueryDialog:create("是否退出217游戏中心？",function(bReTry)
    ...
    --     cc.EventListenerCustom:create("ry_WebClose", handler(self, function ( event )
...
--          print("sdfdfsdfe")
    ...
    :addTo(self)
      ...
      --左上角图标
      self._LeftLog = cc.Sprite:create("base/res/logo.png")
       :setPosition(128,appdf.HEIGHT-70)
       :setScale(0.5)
       :addTo(self)
   ...
   --animal
    self._animal = cc.Sprite:create("base/res/Logingirl.png")
        :setPosition(appdf.WIDTH / 2 -180, appdf.HEIGHT / 2 + 15)
        :addTo(self)
...
--		:addTo(self)
    ...
    self._btnLogonWx:setPosition(appdf.WIDTH / 2 - 230, 134)
    ...
    self._btnLogonAccount:setPosition(appdf.WIDTH / 2 +220, 134)
        ...
        local function loginCallBack ( param )
		    ...
		    if type(param) == "string" and string.len(param) > 0 then
			    ...
			    if ok and type(datatable) == "table" then
                    ...
                    -- action["wCityNum"]          = ""
                    -- action["cbEnjoyType"]       = ""
                    -- action["szNickName"]        = nickname
                    -- action["szIdentity"]        = ""
                    -- action["szEnjoyCode"]       = ""
                    -- action["channelid"]         = self:getChannelID()
                    -- action["szThirdPartyUrl"]    = headurl
                    -- action["IP"] = strtable.cip
                    -- local pargams = {1,3,action}
...
function LogonScene:scheduleCallback(  )
    print("开启啊啊啊啊")
    if self.dddddschedule == nil then
        self.dddddschedule = cc.Director:getInstance():getScheduler():scheduleScriptFunc(function(dt)
            
            yl._clientConnectedCount  = yl._clientConnectedCount +1

            if yl._clientConnectedCount <=2 then
...
function LogonScene:onLogonCallBack(result, message)
    ...
    if result == 1 then --成功
		...
		if self._Operate == 2 then 					--游客登录
			...
			GlobalUserItem.szPassword = GlobalUserItem.PasswordEncryption("com.game217.yx217.pass")
