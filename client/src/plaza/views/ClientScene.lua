...
function ClientScene:onCreate()
	...
	if self._areaTrumpet == nil then
		...
		self._areaTrumpet:addTrugTxt("217游戏中心，提倡健康休闲的游戏理念，严禁赌博、严禁私下交易")
	end
	...
	self._gameListLayer = GameListLayer:create(self)
												:setContentSize(800,470)--(908 + (yl.APPSTORE_VERSION and self._areaCategory:getContentSize().width or 0), 512)
												:setPosition(458, 130)
	...
	--公告按钮
	local btnNotice = self.area_morebutton:getChildByName("btn_notice")
	...
	--播放粒子效果
	local emitter1 = cc.ParticleSystemQuad:create("plaza/testlizi.plist")  
	...
	--sc粒子
	local emitter_sc = cc.ParticleSystemQuad:create("plaza/sc.plist")  
...
function ClientScene:RollingPlaybackEvent()
        ...
        if eventtype == ccui.TouchEventType.ended then
         	if tag == 1 then
				if not self._isClickGame then
					...
					self._gameListLayer:onClickGame(1040)
				end
			elseif tag == 2 then
				if not self._isClickGame then
	                self._gameListLayer._isClickGame = true
			...
			elseif tag == 3 then
				if not self._isClickGame then
	                self._gameListLayer._isClickGame = true
					self._gameListLayer:onClickGame(19)
				end
	...
	self._rollLayout = ccui.Layout:create()
		:setContentSize(cc.size(423,473))
	...
	for i = 1, self._itemmaxcount do 
        ...
        table.insert(self.posindex, (i - 1) * 473)
	end
	self._curbtnTag = 1
	self:layoutAction()
...
function ClientScene:layoutAction()
	--local moveposy = {{0, -473, -946}, {473, 0, -473}, {-473, 473, 0}}
	--local moveposy = {{0, 473, 946}, {946, 0, 473}, {473, 946, 0}}
	local action = cc.RepeatForever:create(cc.Sequence:create(cc.DelayTime:create(5),cc.CallFunc:create(function()
			...
			for i = 1, self._itemmaxcount do 
				...
				btn:runAction(cc.Sequence:create(cc.MoveBy:create(0.2,cc.p(0, -473)),cc.CallFunc:create(function()
					btn:move(cc.p(0,self.posindex[i]))--moveposy[curindex][i]))
					-- local offset = curindex - i
					-- if curindex == maxcount and i == 1 then
					-- 	offset = -1
					-- end
					-- btn:move(cc.p(0,offset * 473))
...
function ClientScene:onTouchEventListener(node)
    ...
    local function onTouchMoved(touch,event)    	
    	...
    	if deltay > 2 then --向上划动
            self.bMove = true
            self.bMoveUp = true
            self._rollLayout:stopAllActions()
    	elseif deltay < -2 then --向下滑动 
            self.bMove = true               
            self.bMoveUp = false
            self._rollLayout:stopAllActions()
    	end
...
function ClientScene:onPassReviewByAndroid(  )
	...
	self.area_morebutton:getChildByName("btn_notice"):setVisible(false)
	self.area_morebutton:getChildByName("btn_service"):setVisible(false)
end
               ...
               print("当前层 = "..tostring(currentLayerTag))
				...
				elseif currentLayerTag == LayerTag.ROOM then
					...
					if self.isQuery == nil then
						self.isQuery = QueryDialog:create("是否退出217游戏中心？",function(bReTry)
							if bReTry == true then
								os.exit(0)
							end
							self.isQuery = nil
...
function ClientScene:onClickBack()
	...
	if currentLayerTag == LayerTag.ROOM_LIST then
		...
		AnimationHelper.moveOutTo(self._areaUserInfo ,0.4, cc.p(self._ptUserInfo.x,self._ptUserInfo.y),function() self._btnBack:setVisible(false) end)
...
function ClientScene:OnAutoGameHide()
  --隐藏喇叭
	...
	AnimationHelper.moveOutTo(self._areaUserInfo ,0.4, cc.p(self._ptUserInfo.x + 150,self._ptUserInfo.y),function() self._btnBack:setVisible(true) end)
...
function ClientScene:onClickGameByAndroidMajia( wKindID )
	...
	AnimationHelper.moveOutTo(self._areaCategory, 0, cc.p(self._ptAreaCategory.x + 1200, self._ptAreaCategory.y))
	AnimationHelper.moveOutTo(self._gameListLayer, 0, cc.p(self._ptGameListLayer.x + 1200, self._ptGameListLayer.y))
	...
	AnimationHelper.moveOutTo(self._areaUserInfo ,0, cc.p(self._ptUserInfo.x + 150,self._ptUserInfo.y),function() self._btnBack:setVisible(true)
...
function ClientScene:onClickGame(wKindID)
	...
	AnimationHelper.moveOutTo(self._areaBottom, 0.4,  cc.p(self._ptAreaBottom.x, self._ptAreaBottom.y - 200))
	AnimationHelper.moveOutTo(self._youngsister, 0.4, cc.p(self._ptYoungSister.x - 600,self._ptYoungSister.y))
	AnimationHelper.moveOutTo(self._areaUserInfo ,0.4, cc.p(self._ptUserInfo.x + 150,self._ptUserInfo.y),function() self._btnBack:setVisible(true)
...
function ClientScene:setWindowsCloseEvent()
	...
	ds.MyNotification:getInstance():registerLuaEventCallBack("WINDWOS_CLOSE",function()
        ...
        self._dialog = QueryDialog:create("是否退出217游戏中心？",function(bReTry)
			        if bReTry == true then
				        os.exit(0)
				    else
				    	self._dialog = nil
...
function ClientScene:requestRollNotice()
	...
	if self._isGunFirst == true then
		...
		jsdata ="217游戏中心，提倡健康休闲的游戏理念，严禁赌博、严禁私下交易"--string.gsub( jsdata,"%s+","")
