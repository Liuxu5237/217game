...
function RoomListLayer:showRoomList(wKindID,bshow)
    ...
    local roomSize          =   cc.size(311, 317)
    ...
    for i = 0, roomCount - 1 do
        ...
        x = btnRoom:getContentSize().width / 2 + (btnRoom:getContentSize().width + 5) * i + 70
...
function RoomListLayer:onClickRoom(wServerID)
    ...
    dump(roomList)
    if self._kindID == 516 or self._kindID == 601  or self._kindID == 19 then
        showPopWait(true,3)
    else
