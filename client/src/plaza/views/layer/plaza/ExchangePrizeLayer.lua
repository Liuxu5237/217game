...
function ExchangePrizeLayer:updatePrizeList(listType)
    ...
    local itemSize      = cc.size(245, 275)
...
function ExchangePrizeLayer:getListItem(listType, index)
    ...
    txtItemName:setPosition(itemSize.width / 2, 130)
    txtItemName:addTo(item)
    
    --商品数量
    local txtItemCount = cc.Label:createWithTTF("剩余:"..itemCount, "fonts/round_body.ttf", 18)
    txtItemCount:setAnchorPoint(0.5, 0.5)
    txtItemCount:setPosition(itemSize.width / 2, 90)
    txtItemCount:setTextColor(cc.c3b(140,239,246))
    ...
    iconPrice:setPosition(iconX, 30)
    txtPrice:setPosition(iconX + priceIconWidth / 2 + priceTextWidth / 2 + 4, 30)
...
function ExchangePrizeLayer:requestExchangeInfoList(listType)
    -- https://interface.game217.com:7417/WS/PropShop.ashx?action=getprizes&type=2	
    ...
    -- https://interface.game217.com:7417/WS/PropShop.ashx?action=getprizes&type=1	
    ...
    --  https://interface.game217.com:7417/WS/PropShop.ashx?action=getprizesrecord&userid=6367&signature=A4490C501370D90D2F730F1B614F538E&time=1525601526
    --  -response-[{"ChangeDate":"2018/5/6 18:10:35","PrizeName":"5万金币","SL":5.00,"DHM":"","BZ":"兑换成功"}]	

    --兑换
    -- https://interface.game217.com:7417/WS/CashPrizes.ashx?param=eyJQcml6ZUlEIjo0LCJKRyI6NSwiVXNlcklEIjoiNjM2NyIsIlRpbWVTdGFtcCI6IjE1MjU2MDE0NjMiLCJTaWduIjoiNTA0QUREQkFGRTVEODRCQUYyOTExMTkzOUVBQTEwNTUiLCJUeXBlIjoyfQ==	
