...
function RankingLayer:tableCellAtIndex(view, idx)   
    ...
    if nil == cell then
        ...
        ccui.TextAtlas:create()
            :setProperty("0","Rank/sp_rank_number.png",19,31,"0")
            :setTag(3)
            :setPosition(45,35)
            :setVisible(false)
            :addTo(btnBg)     
        ...
        :setAnchorPoint(0, 0.5)
        :setPosition(170, 18)
        :setTextColor(cc.c3b(142, 208, 245))
        :addTo(btnBg)
    ...
    --字符串全提出来
    local signstrtable = ExternalFun.utf8StringSplit(item.UnderWrite)
    local signstr = ""
    local signlen = 1
...
function RankingLayer:onClickRankUserInfo(userItem)
    ...
    local signstrtable = ExternalFun.utf8StringSplit(userItem.NickName)
    local signstr = ""
    local signlen = 1
    for i = 1, #signstrtable do
        if signlen >= 12 then
    ...
    local Gameid = self._userInfo:getChildByName("txt_gameid"):setString("（ID："..tostring(userItem.GameID).."）")

    local sex = self._userInfo:getChildByName("sp_sex")
    sex:setPositionX(Gameid:getPositionX() + Gameid:getContentSize().width + 25)
    if userItem.Gender == 0 then
    ...
    :setString(userItem.UnderWrite)

    local txtGold = self._userInfo:getChildByName("txt_gold")
    
    local str = ExternalFun.numberThousands(userItem.Score)
    str = string.gsub(str,",",":")
    txtGold:setString(str)
    local txtwith = txtGold:getContentSize().width
    local txtlen = string.len(str)
