	        ...
	        if bConfirm == true then       
                ...
                elseif device.platform == "android" then
                    ...
                    if ok then
                        ...
                        local filepath = dstpath .. "game217.apk"
                        ...
                        --http://game217.oss-cn-hangzhou.aliyuncs.com/Download/game217.apk
                        downFileAsync(self._androidUpdateUrl,"game217.apk",dstpath,function(main,sub)
                            --下载回调
                ...
                elseif device.platform == "windows" then
                    ...
                    if not getWindowsVersion then
                        ...
                        downFileAsync(self._windowsUpdateUrl,"game217.exe",dstpath,function(main,sub)
                            --下载回调
                            if main == appdf.DOWN_PRO_INFO then --进度信息
                                self:updateProgressBar(sub)
                            elseif main == appdf.DOWN_COMPELETED then --下载完毕
                                self:showProgressBar(false)
                                --安装EXE
                                local fRet = InstallEXE(dstpath.."game217.exe")
                                if fRet then
                                    os.exit(0)
                                end
                            else
...
function WelcomeScene:enterClient()
   ...
   -- yl = nil
    ...
    if device.platform == "windows" then
        ...
        self._txtTips:setString("v " .. appdf.BASE_C_VERSION .. "." .. (self:getApp()._version:getResVersion() or appdf.BASE_C_RESVERSION) .. "." .. appdf.CHANNEL_ID)
    end
	--场景切换
                            ...
                            for i = 1, #updatefolders do
                                ...
                                if cc.PLATFORM_OS_WINDOWS == targetPlatform and true then
                                    if folder == "base" and getInstallationPath then
                                        ...
                                        local index = string.find(installPath,"Game217.exe")
                                        print("============",index)
        ...
        if succeed then --成功
			if LOCAL_DEVELOP == 1 then
                ...
                if device.platform == "windows" and true then
    				...
    				--判断是否需要重新下载完整App
     	        	if self._newVersion and self._newVersion > appdf.BASE_C_VERSION and self._froceUpdate then
                    	--更新客户端
                    	self:updateApp()
                	else
                    	--更新资源
